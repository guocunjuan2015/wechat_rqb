...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    showHoverInfo: function (obj) { // hover 时显示当天信息
      ...
      this.$calendar_today
            .css({left: offset_l + 10, top: offset_t})
            .stop()
            .animate({left: offset_l + 10, top: offset_t, opacity: 1});
    ...
    renderDOM: function () { // 渲染DOM
      ...
      var _weekStr = '<li class="item">周日</li>'+
                      ...
                      '<li class="item">周六</li>';
      var _dateStr = '';
      var _dayStr = '<div class="left-arrow"></div>'+
                    '<p class="num">回款笔数：2笔</p>'+
                 
                    '<p class="money">回款金额：200 元</p>'+
                    '<p class="principal">回款本金：100 元</p>';
